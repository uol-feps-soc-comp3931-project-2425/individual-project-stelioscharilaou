			...
			for(int k=0; k<SS.getSimulationScenarios().length; k++)
			{
				for(int i=0; i<SS.getOrchestratorPolicies().length; i++)
				{
					...
					try
					{
						...
						// Check if Cloud Datacenter is enabled before starting it
						if (SimSettings.getInstance().getNumOfCloudHost() > 0) {
							manager.getCloudServerManager().startDatacenters();
						} else {
							SimLogger.printLine("Cloud Datacenter is disabled. Running Edge-Only Simulation.");
						}
